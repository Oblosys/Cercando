...
tsc -m commonjs -t ES5 --noImplicitAny LucyServer/js/server/LucyServer.ts
if [ "$?" -ne "0" ]; then
...
tsc -m commonjs -t ES5 --noImplicitAny LucyServer/js/client/Locator.ts
if [ "$?" -ne "0" ]; then
...
tsc -m commonjs -t ES5 --noImplicitAny LucyServer/js/tools/UserManager.ts
if [ "$?" -ne "0" ]; then
