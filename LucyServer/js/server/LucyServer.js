...
function initServer() {
    ...
    app.get('/', function (req, res) {
        res.redirect('/locator.html');
    }); // redirect '/' to '/locator.html'
    ...
    app.get('/query/version', function (req, res) {
        child_pr.exec("/Users/martijn/git/Cercando/scripts/generateGitInfo.sh", { cwd: '../..' }, function (error, stdout, stderr) {
...
function trilaterateAllTags() {
    _(state.tagsData).each(function (tag, i) {
        tag.coordinate = { x: i / 10, y: i / 10 };
        tag.distances = _(tag.rssis).map(function (rssi) {
